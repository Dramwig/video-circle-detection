...
def process_cropped(input_data_dir, output_dir, output_width, output_height, str_index = '3'):
    ...
    print(f'裁剪区域：第 {str_index} 个')
    switcher = {
        '1': [[113, 229], [160, 229], [113, 274], [159, 275]],
        '2': [[257, 228], [302, 227], [257, 272], [302, 272]],
        '3': [[449, 223], [494, 223], [449, 268], [494, 268]],
        '4': [[647, 222], [689, 222], [647, 265], [688, 265]],
        '5': [[825, 220], [865, 220], [825, 262], [865, 262]],
        '6': [[981, 218], [1019, 216], [981, 259], [1019, 257]]
    }
    # 获取坐标列表，如果 str_index 不在字典中，则使用默认值
    default_value = [[113, 229], [160, 229], [113, 274], [159, 275]]
    pts1 = np.float32(switcher.get(str_index, default_value))
    ...
    for index, file in tqdm(enumerate(input_data_dir), desc="Cropping", total=len(input_data_dir)):
        ...
        cv2.imwrite(os.path.join(output_dir, 'frame_{:04d}.bmp'.format(index)), cropped_image) # 保存裁剪后的图像到指定目录
...
def process_enhance(input_data_dir, output_dir):
    ...
    for index, file in tqdm(enumerate(input_data_dir), desc="Enhancing", total=len(input_data_dir), unit="enhanced image"):
        ...
        final_image = enhance_image(image)        
        final_image.save(os.path.join(output_dir, 'frame_{:04d}.bmp'.format(index))) # 保存裁剪后的图像到指定目录
    
    files_data = [os.path.join(output_dir, file) for file in os.listdir(output_dir)] # 列出文件夹下的文件路径 
    print(f'保存于：{output_dir}')
    if if_check:
        print("文件夹下的文件路径：", files_data[:3]) # 输出文件路径数组的前三个元素
    return files_data
...
def frame_processing(str_index = ''):
    global if_pause, if_check
    if_pause, if_check = False, False
    print("Processing video...")
    dir_original_data = process_video(video_path, os.path.join(dir_temp, "original"))
    dir_cropped_data = process_cropped(dir_original_data, os.path.join(dir_temp, "cropped"), width, height, str_index = ('3' if str_index == '' else str_index))
    dir_processed_data = process_enhance(dir_cropped_data, os.path.join(dir_temp, f"processed{str_index}"))
